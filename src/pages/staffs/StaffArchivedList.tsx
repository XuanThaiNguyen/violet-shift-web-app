...
const StaffArchivedList = () => {
  ...
  const { data: staffData, isLoading } = useGetArchivedStaffs(filter);
        ...
        case "name":
          return (
            <User
              ...
              description={staff.preferredName ? fullName : ""}
              name={staff.preferredName || fullName || ""}
            >
              {staff.preferredName || fullName || ""}
            </User>
          );
        case "mobile":
          return (
            <p className="text-bold text-small capitalize">
              {staff?.mobileNumber ? staff.mobileNumber : EMPTY_STRING}
            </p>
          );
        case "phone":
          return (
            <p className="text-bold text-small capitalize">
              {staff?.phoneNumber ? staff.phoneNumber : EMPTY_STRING}
            </p>
          );
        case "email":
          return (
            <p className="text-bold text-small capitalize">
              {staff?.email ? staff.email : EMPTY_STRING}
            </p>
          );
        case "address":
          return (
            <p className="text-bold text-small capitalize">
              {staff?.address ? staff.address : EMPTY_STRING}
            </p>
          );
        case "unarchived":
          return (
            <Button
              ...
              size="sm"
              isLoading={isPending}
              onPress={() => mutate({ id: staff.id!, isArchived: false })}
            >
  ...
  return (
    <div className="container mx-auto pt-4">
      <div className="bg-content1 shadow-md rounded-lg p-4">
        <h1 className="text-2xl font-bold">Archived Staffs</h1>
...
export default StaffArchivedList;
